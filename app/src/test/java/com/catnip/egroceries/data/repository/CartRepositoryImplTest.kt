...
class CartRepositoryImplTest {
    ...
    @Test
    fun `create cart empty, product id not null`() {
        runTest {
            val mockProduct = mockk<Product>(relaxed = true)
            coEvery { localDataSource.insertCart(any()) } returns 0
            repository.createCart(mockProduct, 0)
                .map {
                    delay(100)
                    it
                }.test {
    ...
    fun `set cart notes`() {
        coEvery { localDataSource.updateCart(any()) } returns 1
        runTest {
            repository.setCartNotes(mockCart)
                .map {
                    delay(100)
                    it
                }.test {
